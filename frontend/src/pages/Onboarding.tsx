...
import { CURRENCIES, TIMEZONES } from './Account'
...
export default function Onboarding() {
  ...
  async function finish(e: FormEvent) {
    ...
    await api.patchSettings({ timezone, currency, sensitivity, onboarding_complete: true })
  ...
  const frames = [
    ...
    <Card key="desk">
      ...
      <p className="mt-1 text-sm text-[#94A3B8]">Timezone, currency, and how aggressively we flag outliers. You can change this later in Settings.</p>
      <form className="mt-4 space-y-3" onSubmit={(e) => { e.preventDefault(); setStep(2) }}>
        ...
        <div>
          ...
          <select value={currency} onChange={(e) => setCurrency(e.target.value)} className="h-9 w-full rounded border border-[#232F46] bg-[#0B0F17] px-3 text-sm">
