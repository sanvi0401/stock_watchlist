...
import { Button, Card, DataBadge, Delta, ErrorState, SeverityPill, Skeleton } from '../components/ui'
    ...
    <div className="space-y-6">
        ...
        <Card accent="#6366F1">
          ...
          <div className="mt-2 flex items-center gap-3">
            ...
            {q.previous_price != null && !q.first_seen ? (
              <span className="text-sm text-[#94A3B8]">from {fmtPrice(q.previous_price)} {q.baseline_at ? `(${fmtRelative(q.baseline_at)})` : ''}</span>
            ) : null}
          </div>
      ...
      <Card>
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <p className="text-xs text-[#94A3B8]">{q.source} · {fmtDateTime(q.timestamp)} · {MARKET_LABEL[q.market_state] ?? q.market_state}</p>
        </div>
        {series.length < 2 ? (
          ...
          <div className="mt-3 h-56">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={series}>
                ...
                <YAxis domain={['auto', 'auto']} stroke="#94A3B8" width={64} tickFormatter={(v: number) => v.toFixed(0)} />
                <Tooltip
