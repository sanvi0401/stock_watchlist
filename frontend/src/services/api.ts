...
export const api = {
  ...
  me: () => request('/auth/me'),
  forgot: (email: string) =>
    request<{ ok: boolean; dev_reset_token?: string }>('/auth/forgot-password', {
      method: 'POST',
  ...
  notifications: () => request<Array<{ id: number; title: string; body: string; read: boolean; created_at: string; kind: string }>>('/notifications'),
}
