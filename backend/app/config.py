...
KNOWN_INSECURE_SECRETS = {"", "dev-change-me-in-production", "replace-with-a-long-random-string", "test-secret", "secret", "changeme"}
...
def _default_environment() -> str:
    ...
    if os.getenv("VERCEL"): return "production"
    return "development"
def _default_database_url() -> str:
    env = _default_environment()
    if os.getenv("DATABASE_URL"): return os.getenv("DATABASE_URL", "")
    if env in {"development", "test"}: return "sqlite+pysqlite:///./marketwatch.dev.db"
    return ""
class Settings(BaseSettings):
    ...
    def _dev_secret_if_empty(self):
        if self.environment in {"development", "test"} and not self.secret_key: self.secret_key = LOCAL_DEV_SECRET
        return self
    @property
    def is_production(self) -> bool: return self.environment == "production"
    @property
    def is_dev_or_test(self) -> bool: return self.environment in {"development", "test"}
    @property
    def allow_dev_reset_echo(self) -> bool:
        if os.getenv("VERCEL"): return False
        return self.environment in {"development", "test"}
    @property
    def cors_origin_list(self) -> list[str]:
        origins = [o.strip() for o in self.cors_origins.split(",") if o.strip()]
        vercel_url = os.getenv("VERCEL_URL")
        if vercel_url: origins.append(f"https://{vercel_url}")
        prod = os.getenv("VERCEL_PROJECT_PRODUCTION_URL")
        if prod: origins.append(f"https://{prod}")
    ...
    @property
    def frontend_origin(self) -> str: return self.public_url

def validate_settings(cfg: "Settings | None" = None) -> None:
    cfg = cfg or settings
    if not cfg.is_production:
        if not cfg.secret_key: cfg.secret_key = LOCAL_DEV_SECRET
        return
    if cfg.secret_key in KNOWN_INSECURE_SECRETS or len(cfg.secret_key) < 32:
        print("FATAL: production requires SECRET_KEY of at least 32 characters and must not be a committed fallback.", file=sys.stderr)
        raise SystemExit(2)
    url = (cfg.database_url or "").lower()
    if not url or "/tmp/" in url or ":memory:" in url or "sqlite" in url:
        print("FATAL: production requires a persistent DATABASE_URL (Postgres). Ephemeral SQLite is not allowed.", file=sys.stderr)
        raise SystemExit(2)
