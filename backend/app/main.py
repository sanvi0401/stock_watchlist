...
from app.routers import auth, authenticator, changes, dashboard, settings as settings_router, stocks, watchlists
...
def health() -> dict:
    ...
    return {"ok": True, "redis": "up" if get_redis() else "fallback", "cache_ok": redis_ok, "environment": settings.environment, "provider": settings.market_data_provider, "refresh": "background-scheduler" if not os.getenv("VERCEL") else "request-driven"}

@app.get("/market/session")
def market_session() -> dict:
    state = us_equity_session()
    return {"market_state": state, "exchange": "US equities (regular hours approximation, not a full holiday feed)"}
